...
def health():
    """Render/Railway/SnapDeploy pings this to confirm the service is alive."""
...
def generate_report():
    ...
    KPIs, business analysis, the AI-written narrative, and charts.

    Form fields (multipart/form-data):
      - file: the CSV file (required)
      - dataset_type: "Sales" | "Attendance" | "Inventory" | "Finance" | "Employee"

    IMPORTANT: every list-like piece of data below is a JSON ARRAY
    (not an object/dict), specifically so frontend code can safely use
    .map() on it without any Object.entries() conversion. This is the
    exact response shape -- see RESPONSE_SCHEMA.md for the full contract.
    """
    ...
    try:
        ...
        # --- KPIs as an ARRAY of {label, value} ---
        kpis_array = [
            {"label": k, "value": to_native(v)}
            for k, v in kpis.items()
        ]

        # --- Charts as an ARRAY of {title, image} ---
        charts_array = []
        kpi_fig = build_kpi_card_figure(kpis)
        if kpi_fig:
            charts_array.append({"title": "KPI Overview", "image": fig_to_base64(kpi_fig)})
        for title, data in analysis.items():
            if data is None or len(data) == 0:
                continue
            fig = build_chart_figure(title, data)
            charts_array.append({"title": title, "image": fig_to_base64(fig)})

        # --- Analysis as an ARRAY of {title, items: [{label, value}]} ---
        analysis_array = [
            {
                "title": title,
                "items": [{"label": str(idx), "value": to_native(val)} for idx, val in series.items()],
            }
        ...
        return jsonify({
            ...
            "kpis": kpis_array,          # ARRAY -- safe for .map()
            "analysis": analysis_array,  # ARRAY -- safe for .map()
            "ai_report": ai_report,      # object (fixed shape: executive_summary, key_findings[], insights, recommendations[], is_mock)
            "charts": charts_array,      # ARRAY -- safe for .map()
        })
